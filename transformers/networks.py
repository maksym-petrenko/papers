...
class TransformersEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.mh_attention = MultiHeadAttention(num_heads, d_model=d_model, masked=False)
...
class TransformersDecoder(nn.Module):
    ...
    def __init__(self, num_heads: int, d_model: int):
        ...
        self.self_attention = MultiHeadAttention(num_heads, d_model, masked=True)
        self.n1 = LayerNorm(d_model)

        self.cross_attention = MultiHeadAttention(num_heads, d_model, masked=False)
    ...
    def __init__(
        ...
        if tokens_path is not None:
            self.embeddings = Embeddings(
                d_model=d_model, vocab_size=vocab_size, saved_tokens_path=tokens_path
            )
        else:
            self.embeddings = None

        self.encoders = nn.ModuleList(
            [TransformersEncoder(enc_heads, self.d_model) for _ in range(encoder_depth)]
        )
